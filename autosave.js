...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (headerSubtitle) {
    headerSubtitle.innerHTML +=
      ' <span id="autosave-indicator" style="font-size: 12px; opacity: 0.8;">(Autosave enabled)</span>';
  }
  ...
  if (controlPanel) {
    ...
    autosaveStatus.style.cssText =
      "font-size: 12px; margin-top: 10px; color: #666; text-align: right;";
    autosaveStatus.innerHTML =
      'Autosave: <span style="color: #2ecc71;">Enabled</span>';
  ...
  if (!isLocalStorageAvailable()) {
    ...
    if (autosaveIndicator) {
      autosaveIndicator.innerHTML =
        "(Autosave unavailable in private browsing)";
    ...
    if (autosaveStatus) {
      autosaveStatus.innerHTML =
        'Autosave: <span style="color: #e74c3c;">Unavailable</span>';
    }
  }
});
