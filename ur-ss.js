...
document
  .querySelectorAll(
    ".toolbar button:not(#undo-button):not(#redo-button), .table-generator button"
  )
  .forEach((btn) => {
...
addIdentifierButton.addEventListener("click", function () {
  ...
  if (lastTagPos === -1) {
    showNotification(
      "Position cursor inside or right after an HTML tag",
      "error"
    );
    return;
  }
  ...
  const identifierType = prompt(
    "Enter identifier type (class or id):",
    "class"
  );
  if (
    !identifierType ||
    (identifierType !== "class" && identifierType !== "id")
  ) {
    return;
  }

  const identifierValue = prompt(
    `Enter ${identifierType} value:`,
    identifierType === "class" ? "my-class" : "my-id"
  );
